...
bool retro_load_game(const struct retro_game_info *game) {
  ...
  struct retro_input_descriptor desc[] = {
      ...
      {0, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_B,
       "Right Mouse Button"},
      {0, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_A,
       "Left Mouse Button"},
  ...
  if (environ_cb(RETRO_ENVIRONMENT_GET_SYSTEM_DIRECTORY, &sysdir))
    ...
    if (log_cb)
      log_cb(RETRO_LOG_WARN,
             "No System directory specified, using current directory.\n");
    retroSetSystemDir(".");
  }
  ...
  if (environ_cb(RETRO_ENVIRONMENT_GET_SAVE_DIRECTORY, &savedir))
    ...
    if (log_cb)
      log_cb(RETRO_LOG_WARN,
             "No Save directory specified, using current directory.\n");
    retroSetSaveDir(".");
  }
...
void retro_run(void) {
  ...
  if (g_system) {
    ...
    int count = ((Audio::MixerImpl *)g_system->getMixer())
                    ->mixCallback((byte *)buf, 800 * 4);
