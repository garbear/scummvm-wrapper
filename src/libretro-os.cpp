...
static INLINE void blit_uint8_uint16_fast(Graphics::Surface &aOut,
                                          const Graphics::Surface &aIn,
                                          const RetroPalette &aColors) {
...
static INLINE void blit_uint32_uint16(Graphics::Surface &aOut,
                                      const Graphics::Surface &aIn,
                                      const RetroPalette &aColors) {
  for (int i = 0; i < aIn.h; i++) {
    if (i >= aOut.h)
...
static INLINE void blit_uint16_uint16(Graphics::Surface &aOut,
                                      const Graphics::Surface &aIn,
                                      const RetroPalette &aColors) {
  for (int i = 0; i < aIn.h; i++) {
    if (i >= aOut.h)
...
static void blit_uint8_uint16(Graphics::Surface &aOut,
                              const Graphics::Surface &aIn, int aX, int aY,
                              const RetroPalette &aColors, uint32 aKeyColor) {
  for (int i = 0; i < aIn.h; i++) {
...
static void blit_uint16_uint16(Graphics::Surface &aOut,
                               const Graphics::Surface &aIn, int aX, int aY,
                               const RetroPalette &aColors, uint32 aKeyColor) {
  for (int i = 0; i < aIn.h; i++) {
    if ((i + aY) < 0 || (i + aY) >= aOut.h)
...
static void blit_uint32_uint16(Graphics::Surface &aOut,
                               const Graphics::Surface &aIn, int aX, int aY,
                               const RetroPalette &aColors, uint32 aKeyColor) {
  for (int i = 0; i < aIn.h; i++) {
    if ((i + aY) < 0 || (i + aY) >= aOut.h)
...
static INLINE void copyRectToSurface(uint8_t *pixels, int out_pitch,
                                     const uint8_t *src, int pitch, int x,
                                     int y, int w, int h, int out_bpp) {
...
public:
  ...
  OSystem_RETRO(bool aEnableSpeedHack)
      : _mousePaletteEnabled(false), _mouseVisible(false), _mouseX(0),
        _mouseY(0), _mouseXAcc(0.0), _mouseYAcc(0.0), _mouseHotspotX(0),
        _mouseHotspotY(0), _dpadXAcc(0.0), _dpadYAcc(0.0), _dpadXVel(0.0f),
        _dpadYVel(0.0f), _mouseKeyColor(0), _mouseDontScale(false),
        _joypadnumpadLast(8), _joypadnumpadActive(false), _mixer(0),
        _startTime(0), _threadExitTime(10),
...
#ifdef FRONTEND_SUPPORTS_RGB565
    _overlay.create(RES_W_OVERLAY, RES_H_OVERLAY,
  ...
  virtual void initSize(uint width, uint height,
    ...
    _gameScreen.create(width, height,
                       format ? *format
...
public:
  ...
  virtual void updateScreen() {
    ...
    if (_mouseVisible && _mouseImage.w && _mouseImage.h) {
      ...
      case 1:
      case 3:
        blit_uint8_uint16(_screen, _mouseImage, x, y,
                          _mousePaletteEnabled ? _mousePalette : _gamePalette,
                          _mouseKeyColor);
        break;
      case 2:
        blit_uint16_uint16(_screen, _mouseImage, x, y,
                           _mousePaletteEnabled ? _mousePalette : _gamePalette,
                           _mouseKeyColor);
        break;
      case 4:
        blit_uint32_uint16(_screen, _mouseImage, x, y,
                           _mousePaletteEnabled ? _mousePalette : _gamePalette,
                           _mouseKeyColor);
        break;
      }
  ...
  virtual void setMouseCursor(const void *buf, uint w, uint h, int hotspotX,
    ...
    const Graphics::PixelFormat mformat =
        format ? *format : Graphics::PixelFormat::createFormatCLUT8();

    if (_mouseImage.w != w || _mouseImage.h != h ||
        _mouseImage.format != mformat) {
...
#ifdef FRONTEND_SUPPORTS_RGB565
      _screen.create(srcSurface.w, srcSurface.h,
                     Graphics::PixelFormat(2, 5, 6, 5, 0, 11, 5, 0, 0));
#else
      _screen.create(srcSurface.w, srcSurface.h,
                     Graphics::PixelFormat(2, 5, 5, 5, 1, 10, 5, 0, 15));
...
#define PI 3.141592653589793238
  ...
  void processMouse(retro_input_state_t aCallback, int device,
    ...
    float screen_adjusted_cursor_speed =
        ...
        320.0f; // Dpad cursor speed should always be based off a 320 wide
                // screen, to keep speeds consistent
    float adjusted_cursor_speed = (float)BASE_CURSOR_SPEED *
                                  gampad_cursor_speed *
                                  screen_adjusted_cursor_speed;
    float inverse_acceleration_time =
        (gamepad_acceleration_time > 0.0)
            ? (1.0 / 60.0) * (1.0 / gamepad_acceleration_time)
            : 1.0;
    int dpad_cursor_offset;
    double rs_radius, rs_angle;
    unsigned numpad_index;

    static const uint32_t retroButtons[2] = {RETRO_DEVICE_ID_MOUSE_LEFT,
    ...
    static const unsigned gampad_key_map[8][4] = {
        {RETRO_DEVICE_ID_JOYPAD_X, (unsigned)Common::KEYCODE_ESCAPE,
         (unsigned)Common::ASCII_ESCAPE, 0}, // Esc
        {RETRO_DEVICE_ID_JOYPAD_Y, (unsigned)Common::KEYCODE_PERIOD, 46,
         0}, // .
        {RETRO_DEVICE_ID_JOYPAD_L, (unsigned)Common::KEYCODE_RETURN,
         (unsigned)Common::ASCII_RETURN, 0}, // Enter
        {RETRO_DEVICE_ID_JOYPAD_R, (unsigned)Common::KEYCODE_KP5, 53,
         0}, // Numpad 5
        {RETRO_DEVICE_ID_JOYPAD_L2, (unsigned)Common::KEYCODE_BACKSPACE,
         (unsigned)Common::ASCII_BACKSPACE, 0}, // Backspace
        {RETRO_DEVICE_ID_JOYPAD_L3, (unsigned)Common::KEYCODE_F10,
         (unsigned)Common::ASCII_F10, 0}, // F10
        {RETRO_DEVICE_ID_JOYPAD_R3, (unsigned)Common::KEYCODE_KP0, 48,
         0}, // Numpad 0
        {RETRO_DEVICE_ID_JOYPAD_SELECT, (unsigned)Common::KEYCODE_F7,
         (unsigned)Common::ASCII_F7,
         RETROKMOD_CTRL}, // CTRL+F7 (virtual keyboard)
    };
    ...
    joy_y = aCallback(0, RETRO_DEVICE_ANALOG, RETRO_DEVICE_INDEX_ANALOG_LEFT,
    ...
    if (joy_x > analog_deadzone || joy_x < -analog_deadzone) {
      ...
      // Update accumulator
      analog_amplitude_x =
          (float)joy_x / (float)(ANALOG_RANGE - analog_deadzone);
    ...
    if (joy_y > analog_deadzone || joy_y < -analog_deadzone) {
      ...
      // Update accumulator
      analog_amplitude_y =
          (float)joy_y / (float)(ANALOG_RANGE - analog_deadzone);
      if (analog_response_is_quadratic) {
    ...
    if (device == RETRO_DEVICE_JOYPAD) {
      bool dpadLeft =
          aCallback(0, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_LEFT);
      bool dpadRight =
          aCallback(0, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_RIGHT);
      bool dpadUp =
          aCallback(0, RETRO_DEVICE_JOYPAD, 0, RETRO_DEVICE_ID_JOYPAD_UP);
      bool dpadDown =
    ...
    int p_press =
        aCallback(0, RETRO_DEVICE_POINTER, 0, RETRO_DEVICE_ID_POINTER_PRESSED);
    ...
    for (int i = 0; i < 8; i++) {
      ...
      if (down != _joypadkeyboardButtons[i]) {
        ...
        processKeyEvent(state, gampad_key_map[i][1],
                        (uint32_t)gampad_key_map[i][2],
                        (uint32_t)gampad_key_map[i][3]);
      }
    }

    // Gamepad right stick numpad emulation
    joy_rx = aCallback(0, RETRO_DEVICE_ANALOG, RETRO_DEVICE_INDEX_ANALOG_RIGHT,
                       RETRO_DEVICE_ID_ANALOG_X);
    joy_ry = aCallback(0, RETRO_DEVICE_ANALOG, RETRO_DEVICE_INDEX_ANALOG_RIGHT,
                       RETRO_DEVICE_ID_ANALOG_Y);
    ...
    if (joy_rx != 0 || joy_ry != 0) {
      analog_amplitude_x =
          (float)joy_rx / (float)(ANALOG_RANGE - analog_deadzone);
      analog_amplitude_y =
          (float)joy_ry / (float)(ANALOG_RANGE - analog_deadzone);

      // Convert to polar coordinates: part 1
      rs_radius = sqrt((double)(analog_amplitude_x * analog_amplitude_x) +
                       (double)(analog_amplitude_y * analog_amplitude_y));

      // Check if radius is above threshold
      if (rs_radius > 0.5) {
        // Convert to polar coordinates: part 2
        rs_angle =
            atan2((double)analog_amplitude_y, (double)analog_amplitude_x);
        ...
        if (numpad_index != _joypadnumpadLast) {
          ...
          if (_joypadnumpadActive)
            processKeyEvent(false, gampad_numpad_map[_joypadnumpadLast][0],
                            (uint32_t)gampad_numpad_map[_joypadnumpadLast][1],
                            0);

          // Set new key
          processKeyEvent(true, gampad_numpad_map[numpad_index][0],
                          (uint32_t)gampad_numpad_map[numpad_index][1], 0);
        ...
        processKeyEvent(false, gampad_numpad_map[_joypadnumpadLast][0],
                        (uint32_t)gampad_numpad_map[_joypadnumpadLast][1], 0);
  ...
  void processKeyEvent(bool down, unsigned keycode, uint32_t character,
    ...
    if (ev.kbd.ascii >= 97 && ev.kbd.ascii <= 122 &&
...
void retroProcessMouse(retro_input_state_t aCallback, int device,
                       float gamepad_cursor_speed,
                       float gamepad_acceleration_time,
                       bool analog_response_is_quadratic, int analog_deadzone,
                       float mouse_speed) {
  dynamic_cast<OSystem_RETRO *>(g_system)->processMouse(
      aCallback, device, gamepad_cursor_speed, gamepad_acceleration_time,
...
void retroKeyEvent(bool down, unsigned keycode, uint32_t character,
                   uint16_t key_modifiers) {
